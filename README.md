...
```
{
  ...
  "treatments_classified": [ {id, text, label, category, date, source_treatment_ids}, ... ],  // frozen; classifier retired
...
│   ├── treatment_identity.py # Deterministic treatment identity + splitting
...
infer a trend. The server also returns analyte-level counts for every missing
chart requirement, and the explorer shows each numeric result's coarse
within/above/below position against that result's own report range without
comparing values or methods. CgA/S-CgA/P-CgA are one naming family, but this
alias grouping grants no comparability. The response carries both revisions plus opaque authority tokens;
...
|---|---|---|
| Review current priorities | **Today** | Shows assessment freshness, the expanded latest assessment, its confidence/PRRT/CgA badges directly under the heading, key concern, and recommended next steps first; the freshness banner always owns the sole assessment action — **Refresh assessment** when stale (hidden conclusions remain unavailable) and **Regenerate assessment** when up to date, so a current assessment can still be rerun on demand. Bounded recorded update times/active-alert count follow with shared secondary actions, then treatment status, symptoms, follow-ups, appointment preparation, and lower-priority tracked research |
| Recover from a failed load | Any view → failure banner | The banner names the part that failed — **Imaging could not be loaded**, and both parts when two fail. It keeps the wider **Patient data could not be loaded** when the failure is wider: an area with no name of its own, three or more at once, or any failure that cleared the browser's whole copy of the record. Sign-in, denied access, blocked address and offline keep their own wording |
...
| Record or review symptom episodes | **Today** → **Symptoms logged** or **Patient** → **Symptoms** | Today carries a one-line entry — a description, an optional severity and **Record** — for the common case of noting what she felt today; it opens and submits the same guarded creation the full dialog uses, so nothing it can record bypasses a check the dialog makes, and the dialog stays on screen with the wording in it if the record changed underneath or the connection dropped. The full dialog remains for onset, timing, frequency, triggers, notes and follow-up linkage. Patient adds explicit fact editing, resolution, resolved review, source observations you cannot edit, and atomic existing/manual follow-up linkage. Routine capability boilerplate is omitted while caregiver-entered attribution, actionable errors, and the no-inference behavior remain unchanged |
| Review treatment information | **Today** → **Treatment status** or **Patient** → **Treatments** | Today never appears empty when recorded treatment rows exist. Patient's default Overview shows explicitly reviewed current/planned records, then every raw treatment row with presentation-only **not linked / fully linked / partly linked** status derived solely from explicit component associations, then finished/past records. Each of those groups is listed newest-first: a status record by the date its own status is about (planned date when planned, start date when current, stop date — falling back to the start date — when finished or past), and a raw row by most recently recorded, because raw rows carry no date. Year-only and month-only dates sort at the start of the period they name, undated records hold a fixed last position, and equal keys break on row identity, so the order is total and stable; Today uses the same order. Ordering is presentation only and no date is parsed from wording, borrowed from a linked record, or taken from generated text. Only unresolved rows count as needing timing/status review. When no current or planned record exists the empty list names the actions that populate it, offering only routes that currently exist, rather than reading as a dead end. Each full recorded row carries exactly one action, **Record status**, which opens the same status-record dialog pre-filled with that row's wording and its components pre-ticked for linking, and each **Mentions in source documents** row carries the same action pre-filled with that mention's exact observed wording only — mentions carry no temporal data and their `operation` is list-membership, not a clinical event. Both use one creation contract; no status, date, or ending detail is ever preselected, so the dialog cannot be saved until the caregiver chooses the status. That dialog shows only the status, the treatment wording and the three dates, with the nine optional wording boxes behind one closed **Add more detail (optional)** disclosure that opens by itself whenever the record or a restored draft already has content and keeps a filled-in count on its summary even while collapsed. Compact Today cards stay read-only. Source mentions and differences stay separate; nothing is promoted, hidden, merged, deduplicated, or assigned lifecycle automatically. Stored NET/Care-generated compatibility rows are retained and still projected by the API but have no UI surface |
| See the latest research batch | **Today** → **Research being tracked** | A deferred guarded load fills this lower-priority card after primary status/assessment loading begins. It leads with how many research entries are tracked in total, split into trials and papers, then shows the first three latest-batch entries with exact totals/omissions; membership is passive and never unread/review state. Every count is arithmetic over the projected rows — nothing there judges relevance or eligibility |
...
| Reconcile or correct an import | **Activity** → select that document-processing item, or **Today** → **Latest document import** → **Open Activity** | Shows **What this document changed** independently of narrative-report retention, with human source/exact-wording links; correct/remove a value or safely undo the document's direct structured changes. The Today shortcut opens that import's own activity record, resolved from the intake job stored on the source document; when retention has already removed that record the Activity list opens instead and says so, and the document and everything imported from it are untouched |
| Review complete biomarker history | **Patient** → **Biomarkers** | Opens on **Latest result for each biomarker**: one line per biomarker showing its newest recorded result, unit, reported flag, date and reference range, with **Search biomarkers** filtering by displayed name or recorded alias. The history selector defaults to Chromogranin A when available; S-CgA/P-CgA are naming aliases only and never infer specimen or comparability. A line's colour and always-visible comparison wording repeat only the server's comparison against that report's own range, and exact same-date ties disclose how many results carry the date. **Show history** selects the biomarker and moves focus to its compact five-column table; reference range, specimen, assay/method, date kind, charting status and authenticated source text remain behind **Show detail**. A separate chronological sequence shows each numeric result's coarse position against its own parseable report range without calling it a trend or comparing magnitude. Isolated point charts appear only where the server declares an exact comparable group; otherwise server-computed diagnostics name the missing unit, exact collection/result date, specimen, assay/method, or parseable range while every observation remains in the table |
...
| Refresh the assessment | **Today** → freshness banner → **Regenerate assessment** (**Refresh assessment** when stale, **Generate assessment** when none exists) | Queues the manual summary job: treatments are re-classified, then the executive summary is regenerated and saved. If classification cannot be certified, the job still generates and saves the assessment and finishes with a warning; treatment rows fall back to the raw records shown under Treatment status |
| Run a research-only sweep | Header → **↻ Run digest**, or **Activity** → **Run digest** | Orchestrator runs without new input; new trials/papers added. Both controls start the same job and are disabled together while it is being submitted |
| Record an oncologist's judgment | **Appointments** → **Clinical notes** | Becomes a hard constraint for future runs |
