...
|---|---|---|
| Review current priorities | **Today** | Shows assessment freshness, the expanded latest assessment, key concern, and recommended next steps first; the freshness banner always owns the sole assessment action — **Refresh assessment** when stale (hidden conclusions remain unavailable) and **Regenerate assessment** when up to date, so a current assessment can still be rerun on demand. Bounded recorded update times/active-alert count follow with shared secondary actions, then treatment status, symptoms, follow-ups, appointment preparation, and lower-priority tracked research |
