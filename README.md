...
| Refresh the assessment | **Today** → freshness banner → **Refresh assessment** (or **Generate assessment**) | Queues the manual summary job: treatments are re-classified, then the executive summary is regenerated and saved. If classification cannot be certified, the job still generates and saves the assessment and finishes with a warning; treatment rows fall back to the raw records shown under Treatment status |
