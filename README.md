...
| Review complete biomarker history | **Patient** → **Biomarkers** | Opens on **Latest result for each biomarker**: one line per biomarker showing its newest recorded result, unit, reported flag, date and reference range, with a **Search biomarkers** box that filters by displayed name or recorded alias. A line's colour repeats only the comparison the server already made against the range printed in the report, and that comparison is written out beside it, so the browser never decides whether a result is abnormal. **Show history** below any line selects that biomarker in the full table and moves focus to it. The table keeps every raw observation, now compacted to date, result, reference and reported flag, with the date type, value type, flag source, specimen/assay/method, charting status and source/evidence identity behind a per-row **Show detail** toggle. Isolated point charts appear only where the server declares an exact comparable group |
