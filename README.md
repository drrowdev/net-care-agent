...
| Review treatment information | **Today** → **Treatment status** or **Patient** → **Treatments** | Today never appears empty when recorded treatment rows exist. Patient's default Overview shows explicitly reviewed current/planned records, every recorded treatment row whose timing/status is not yet reviewed, then finished/past records. Source mentions, differences, and collapsed NET/Care compatibility notes stay separate; nothing is promoted, merged, deduplicated, or assigned lifecycle automatically |
