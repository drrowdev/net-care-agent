...
| Horizontal timeline restored at the cost of accessibility | The removed SVG graph was keyboard-unreachable (hover-only tooltips), had a 400px floor that overflowed a phone, truncated labels to 19 characters, and used browser-locale English month names. Its actual shape — one row per event, each plotted at its true position on a shared horizontal axis, with a today line — is kept and rebuilt on ordinary markup: a per-row `aria-hidden` plot strip carries position, the `<li>` carries the text. One row per event is what keeps a marker aligned with its own words and removes any need to shorten a label. Month precision draws a band across the named month rather than a point on an invented day, past is stated in words as well as styling, and below the existing 720px breakpoint the strips are dropped. `tests/test_summary_timeline_ui.py` pins zero page and track overflow, marker/row alignment, and the `<time datetime>` contract at 1280/768/360. |
| Generated prose reaches the screen through many renderers | Localising dates field by field left the model's own sentences untouched wherever they were not assessment fields. Chat replies, generated questions and their rationales, the research report body, and the timeline's own date qualifier all now pass through `fmtProseDates` before markup, so one deterministic transcription covers every generated surface rather than each one remembering. |
