...
def test_unversioned_migration_records_log_entry():
    ...
    assert log[13]["id"] == "0014_add_research_disposition_authority"
    assert log[14]["id"] == "0015_add_profile_revision_authority"
...
def test_v15_backfills_only_truly_missing_profile_revision_losslessly():
    from agent.migrations import apply_migrations
    ...
    base = {
        ...
        "workflow_revision": 9,
        "patient": {
            "current_treatments": ["synthetic one", "synthetic one"],
            "current_treatment_records": [
                {"id": "component-one", "source_order": 0, "unknown": {"keep": True}},
                {"id": "component-two", "source_order": 1, "unknown": {"keep": True}},
            ],
        },
        "clinical_judgments": [
            {"id": "judgment-one", "source": "feedback", "text": "Synthetic judgment"}
        ],
        "treatments_classified": [
            {
                "text": "Synthetic generated context",
                "label": "Synthetic label",
                "category": "active",
                "date": "2020",
            }
        ],
        "unknown_top": [{"keep": None}, {"keep": None}],
    }
    preserved = copy.deepcopy(base)

    migrated = apply_migrations(base)
    once = copy.deepcopy(migrated)

    assert migrated["schema_version"] == 15
    assert migrated["profile_revision"] == 0
    assert migrated["workflow_revision"] == preserved["workflow_revision"]
    for key, value in preserved.items():
        if key != "schema_version":
            assert migrated[key] == value
    assert apply_migrations(migrated) == once

    for existing in (None, "invalid", -1, True, 42):
        profile = {"schema_version": 14, "profile_revision": existing, "patient": {}}
        assert apply_migrations(profile)["profile_revision"] == existing
